...
public class EDCStagingFragment extends Fragment implements RecyclerTouchListener.OnItemClickListener {

    public static final String BUNDLE_EDC_ITEM_KEY = "app.rbzeta.edcimplementationtracker.fragment.BUNDLE_EDC_ITEM_KEY";
    public static final String INTENT_EDC_ITEM_KEY = "app.rbzeta.edcimplementationtracker.fragment.INTENT_EDC_ITEM_KEY";
    private static final String INTENT_TAB_TYPE_KEY = "app.rbzeta.edcimplementationtracker.fragment.INTENT_TAB_TYPE_KEY";

    private static final int REQUEST_CODE_SUBMIT_IMPL = 10;
    private static final int TAB_STAGING = 0;
    ...
    public static EDCStagingFragment newInstance(Bundle bundle){
        EDCStagingFragment frag = new EDCStagingFragment();
    ...
    private void fetchEDCDataFromServer(int page) {
        ...
        Observable<EDCResponseMessage> observable = (Observable<EDCResponseMessage>)
                service.getPreparedObservable(
                        service.getNetworkAPI().getEDCListStagingFromServer(page,session.getUserBranchId(),currentType),
    ...
    public void onItemClick(View view, int position) {
        ...
        //Toast.makeText(getContext(), "Item position "+position+" clicked", Toast.LENGTH_SHORT).show();
    }
    ...
    private void openSubmitActivity(int position) {
        ...
        //intent.putExtra(INTENT_TAB_TYPE_KEY, TAB_STAGING);
        intent.putExtra("TAB_TYPE", TAB_STAGING);
        startActivityForResult(intent,REQUEST_CODE_SUBMIT_IMPL);
    }

    @Override
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_SUBMIT_IMPL){
            if (resultCode == RESULT_OK){
                if (data.getExtras().getBoolean(SubmitActivity.MSG_KEY_SUBMIT_EDC_IMPL_RESULT)){
                   loadInitEDCDataFromServer();
                }
            }
        }
