...
public class EDCCompleteFragment extends Fragment implements RecyclerTouchListener.OnItemClickListener{
    ...
    private static final int REQUEST_CODE_SUBMIT_VER = 20;
    ...
    public static EDCCompleteFragment newInstance(Bundle bundle){
        EDCCompleteFragment frag = new EDCCompleteFragment();
    ...
    private void fetchEDCDataFromServer(int page) {
        ...
        Observable<EDCResponseMessage> observable = (Observable<EDCResponseMessage>)
                service.getPreparedObservable(
                        service.getNetworkAPI().getEDCListImplementedFromServer(page,session.getUserBranchId(),currentType),
    ...
    public void onItemClick(View view, int position) {
        openVerifyActivity(position);
    }

    private void openVerifyActivity(int position) {
        EDCItem item = edcItems.get(position);
        Intent intent = new Intent(getActivity(), SubmitActivity.class);
        intent.putExtra(INTENT_EDC_ITEM_KEY,item);
        intent.putExtra("TAB_TYPE", TAB_COMPLETED);
        startActivityForResult(intent,REQUEST_CODE_SUBMIT_VER);
    }

    @Override
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_SUBMIT_VER){
            if (resultCode == RESULT_OK){
                if (data.getExtras().getBoolean(SubmitActivity.MSG_KEY_SUBMIT_EDC_VER_RESULT)){
                    loadInitEDCDataFromServer();
                }
            }
        }
